...
3. `pleer.js` отвечает за работоспособность панели воспроизведение внизу плеер
4. `playlist.js` отвечает за работу с динамическим плейлистом в корне
5. `files.js` словарь-дерево разделов в разделах и музыкальных файлов, начиная с `audio/`
6. `old_files.js` прошлый `files.js`, на случай утери или повреждения нынешнего (музыку слушать то хочется во время восстановления)
7. `scripts/` содержит скрипты для сборки `files.js` и `img/`, написан на python потому что удобно
